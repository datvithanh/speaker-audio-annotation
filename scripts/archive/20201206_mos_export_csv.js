...
(async () => {
  ...
  for (const [index, audio] of audios.entries()) {
    ...
    const testName =
      testId === firstMosTest ? firstMosTest.name : secondMosTest.name;

    const pieceOfData = await Promise.all(
      users.map(async ({ userId, point, listens, lastUpdate }) => {
  ...
  fs.appendFileSync(
    'scripts/mos.csv',
    'test,audio_id,link,voice,sentence,listens,email,name,point,last_updated_at\n',
  );
